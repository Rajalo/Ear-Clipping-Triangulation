...
/**
 * Stores information about a face, either the polygon or the triangles made from it
 */
public class Face {
    ...
    /**
     * Checks if each vertex is intitially an ear
     */
    public void initializeEars()
    {
        for (int i = 0; i < vertices.size(); i++)
        {
            if (diagonal(vertices.get((i-1+vertices.size())%vertices.size()),vertices.get((i+1)%vertices.size())))
                vertices.get(i).ear = true;
        }
    }
    /**
    ...
    public void checkEar(int v0)
    {
        v0 = (v0 +this.vertices.size())%this.vertices.size();
        this.vertices.get(v0).ear = diagonal(this.vertices.get((v0-1+vertices.size())%vertices.size()),this.vertices.get((v0+1)%vertices.size()));
    }
    /**
    ...
    public void paintFill(Graphics g, Color color)
    {
        g.setColor(color);
        int[][] coords = Vertex.verticesToInts(this.vertices);
        g.fillPolygon(coords[0], coords[1], coords[0].length);
